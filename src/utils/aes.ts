/*
 * AES-GCM encryption/decryption module from:
 ...
 */
export async function aes_gcm_encrypt(plaintext: string, password: string): Promise<string> {
 ...
 */
export async function aes_gcm_decrypt(ciphertext: string, password: string): Promise<string> {
    // encode password as UTF-8
    const pwUtf8 = new TextEncoder().encode(password);
    // hash the password
    const pwHash = await crypto.subtle.digest('SHA-256', pwUtf8);
